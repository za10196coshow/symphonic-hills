(() => {
  ...
  const renderAreas = (data) => {
    document.getElementById('area-comparison').innerHTML = data.areas.map((item) => `<article class="area-card${item.reference ? ' reference-card' : ''}"><strong class="sqm">${esc(item.sqm)}<small>㎡</small></strong><small>${esc(item.label)}</small><div class="room-plan">${item.rooms.map((room) => `<i>${esc(room)}</i>`).join('')}</div></article>`).join('');
  };
  ...
  const renderSources = (data) => {
    document.getElementById('source-list').innerHTML = data.items.map((item) => `<a href="${esc(item.url)}" target="_blank" rel="noopener noreferrer"><span>${esc(item.title)}</span><span>開く</span></a>`).join('');
  };
  ...
  const openSource = (trigger) => {
    ...
    dialogContent.innerHTML = `<h2>${esc(title)}</h2><p>${esc(levelInfo?.label || '')}</p>${item?.note ? `<p>${esc(item.note)}</p>` : ''}<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">出典ページを開く</a>`;
    dialog.showModal();
  };
